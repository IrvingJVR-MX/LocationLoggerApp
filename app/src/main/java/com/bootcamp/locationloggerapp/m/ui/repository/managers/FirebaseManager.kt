...
class FirebaseManager(private val db: FirebaseFirestore) : FirebaseDataSource {
    ...
    override suspend fun <T : Any> addDocumentWithId(document: T, collection: FirebaseCollections, id: String): Response<String> {
        return try {
    ...
    override suspend fun getPostCollectionWithId(collection: FirebaseCollections, id: String): Response<MutableList<Post>> {
        return try {
    ...
    override suspend fun updateCollectionWithId(collection: FirebaseCollections, id: String, name: String, profilePhotoUrl: String, profilePhotoPath: String, backgroundPhotoUrl:String, backgroundPhotoPath: String): Response<Boolean> {
        return try {
            var result = false
            db.collection(collection.toString())
                .document(id)
                .update("name",name,"backgroundPhotoUrl",backgroundPhotoUrl,"backgroundPhotoPath",backgroundPhotoPath,"profilePhotoPath",profilePhotoPath, "profilePhotoUrl",profilePhotoUrl)
    ...
    override suspend fun deleteCollectionWithId(collection: FirebaseCollections, id: String): Response<Boolean> {
        return try {
            var result = false
            db.collection(collection.toString())
                .document(id)
    ...
    override suspend fun getUserInfo(collection: FirebaseCollections, id: String): Response<MutableList<User>> {
        return try {
                    ...
                    db.collection(FirebaseCollections.Users.toString())
                        ...
                        .addOnSuccessListener { result ->
                            for (document in result) {
                                ...
                                val postDetail = PostDetail(post.id, post.userId, post.title, post.description,
                                             post.photoUrl, post.placeName, post.geoPoint, userName as String?, userPhotoUrl as String?)
